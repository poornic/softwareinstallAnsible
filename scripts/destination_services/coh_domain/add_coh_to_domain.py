...
def createCoherenceCluster(cfgmap):
    ...
    set('ClusteringMode', 'multicast')
    set('MulticastListenAddress', cfgmap['multicast-address'])
    set('MulticastListenPort', int(cfgmap['multicast-port']))
...
wls_user = props["wls"]["admin"]["username"]
wls_pass = props["wls"]["admin"]["password"]
wls_url = props["wls"]["admin"]["url"]
wls_domain_dir = props["wls"]["admin"]["domain_dir"]

readDomain(wls_domain_dir)

# Create the Clusters
cd("/")
clusterKeys = props["wls"]["clusters"]
for clusterKey in clusterKeys:
    createCluster(props["wls"]["clusters"][clusterKey])
